...
def plot_accuracy_aggregated(cfg, results_dir, plots_dir):
    for engine, models in cfg["models"].items():
        ...
        save(fig, engine_dir, f"accuracy_overall_{engine}.png")
...
def plot_accuracy_by_type_aggregated(cfg, results_dir, plots_dir, color_map):
    for engine, models in cfg["models"].items():
        ...
        ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3, frameon=False)
        engine_dir = os.path.join(plots_dir, engine)
        save(fig, engine_dir, f"accuracy_by_type_{engine}.png")
...
def plot_latency_aggregated(cfg, results_dir, plots_dir, color_map):
    for engine, models in cfg["models"].items():
        ...
        ax.legend(loc='upper left', bbox_to_anchor=(1, 1), frameon=False)
        engine_dir = os.path.join(plots_dir, engine)
        save(fig, engine_dir, f"latency_{engine}.png")
...
def plot_power_aggregated(cfg, results_dir, plots_dir, color_map):
    for engine, models in cfg["models"].items():
        ...
        ax.legend(loc='upper left', bbox_to_anchor=(1, 1), frameon=False)
        engine_dir = os.path.join(plots_dir, engine)
        save(fig, engine_dir, f"power_gpu_{engine}.png")
...
def plot_temperature_aggregated(cfg, results_dir, plots_dir, color_map):
    for engine, models in cfg["models"].items():
        ...
        ax.legend(loc='upper left', bbox_to_anchor=(1, 1), frameon=False)
        engine_dir = os.path.join(plots_dir, engine)
        save(fig, engine_dir, f"temperature_{engine}.png")
...
def plot_accuracy_vs_temp_aggregated(cfg, results_dir, plots_dir, color_map):
    for engine, models in cfg["models"].items():
        ...
        ax.legend(loc='upper left', bbox_to_anchor=(1, 1), frameon=False)
        engine_dir = os.path.join(plots_dir, engine)
        save(fig, engine_dir, f"acc_vs_temp_{engine}.png")
...
def plot_energy_cdf_aggregated(cfg, results_dir, plots_dir, color_map):
    for engine, models in cfg["models"].items():
        ...
        engine_dir = os.path.join(plots_dir, engine)
        save(fig, engine_dir, f"energy_cdf_{engine}.png")
...
def plot_energy_aggregated(cfg, results_dir, plots_dir, color_map):
    for engine, models in cfg["models"].items():
        ...
        ax.legend(loc='upper left', bbox_to_anchor=(1, 1), frameon=False)
        engine_dir = os.path.join(plots_dir, engine)
        save(fig, engine_dir, f"energy_per_prompt_{engine}.png")

# ---------------- Family‑based aggregated plots ---------------- #

def load_all_data_by_family(base_results, config):
    """Return nested dict: family -> engine -> model_name -> list of DataFrames"""
    family_engine_data = {}
    run_dirs = get_run_dirs(base_results)
    for run_dir in run_dirs:
        for engine, models in config["models"].items():
            for model_name, filename in models.items():
                ...
                if family not in family_engine_data:
                    family_engine_data[family] = {}
                if engine not in family_engine_data[family]:
                    family_engine_data[family][engine] = {}
                if model_name not in family_engine_data[family][engine]:
                    family_engine_data[family][engine][model_name] = []
                family_engine_data[family][engine][model_name].append(df)
    return family_engine_data

def plot_accuracy_family(family_data, base_plots_dir, color_map):
    for family, engines in family_data.items():
        for engine, models in engines.items():
            ...
            table.set_fontsize(11)
            table.scale(1.2, 1.8)
            ax.set_title(f"{family} - {engine} - Overall Aggregated Accuracy",
                         fontsize=14, weight="bold", pad=24)
            family_engine_dir = os.path.join(base_plots_dir, family.lower(), engine)
            save(fig, family_engine_dir, f"accuracy_overall_{engine}.png")
        ...
        for engine, models in engines.items():
            ...
            for i, model_name in enumerate(model_names):
                ...
                ax.bar(x + offset, means, width, yerr=stds, capsize=3,
                       label=model_name, color=color_map.get(model_name))
            ax.set_xticks(x)
            ax.set_xticklabels(all_types, rotation=15)
            ax.set_title(f"{family} - {engine} - Accuracy by Question Type (Mean ± Std)", weight="bold")
            ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3, frameon=False)
            family_engine_dir = os.path.join(base_plots_dir, family.lower(), engine)
            save(fig, family_engine_dir, f"accuracy_by_type_{engine}.png")
    ...
    for family, engines in family_data.items():
        for engine, models in engines.items():
            ...
            for model_name, dfs in models.items():
                ...
                ax.fill_between(iters, mean_lat - std_lat, mean_lat + std_lat, alpha=0.2, color=color)
            ax.set_title(f"{family} - {engine} - Latency (Mean ± Std)", weight="bold")
            ax.legend(loc='upper left', bbox_to_anchor=(1, 1), frameon=False)
            family_engine_dir = os.path.join(base_plots_dir, family.lower(), engine)
            save(fig, family_engine_dir, f"latency_{engine}.png")
    ...
    for family, engines in family_data.items():
        for engine, models in engines.items():
            ...
            for model_name, dfs in models.items():
                ...
                ax.fill_between(range(min_len), mean_p - std_p, mean_p + std_p, alpha=0.2, color=color)
            ax.set_title(f"{family} - {engine} - GPU Power (Mean ± Std)", weight="bold")
            ax.legend(loc='upper left', bbox_to_anchor=(1, 1), frameon=False)
            family_engine_dir = os.path.join(base_plots_dir, family.lower(), engine)
            save(fig, family_engine_dir, f"power_gpu_{engine}.png")
    ...
    for family, engines in family_data.items():
        for engine, models in engines.items():
            ...
            for model_name, dfs in models.items():
                ...
                ax.fill_between(range(min_len), mean_t - std_t, mean_t + std_t, alpha=0.2, color=color)
            ax.set_title(f"{family} - {engine} - Max Temperature (Mean ± Std)", weight="bold")
            ax.legend(loc='upper left', bbox_to_anchor=(1, 1), frameon=False)
            family_engine_dir = os.path.join(base_plots_dir, family.lower(), engine)
            save(fig, family_engine_dir, f"temperature_{engine}.png")
    ...
    for family, engines in family_data.items():
        for engine, models in engines.items():
            ...
            for model_name, dfs in models.items():
                ...
                all_df["temp_bin"] = all_df["max_gpu_temp_c"].round()
                grouped = all_df.groupby("temp_bin")["correct"].agg(['mean', 'std']) * 100
                ax.errorbar(grouped.index, grouped['mean'], yerr=grouped['std'], marker='o', capsize=3,
                            label=model_name, color=color_map.get(model_name))
            ax.set_title(f"{family} - {engine} - Accuracy vs Temperature (Mean ± Std)", weight="bold")
            ax.legend(loc='upper left', bbox_to_anchor=(1, 1), frameon=False)
            family_engine_dir = os.path.join(base_plots_dir, family.lower(), engine)
            save(fig, family_engine_dir, f"acc_vs_temp_{engine}.png")
...
def plot_energy_cdf_family(family_data, base_plots_dir, color_map):
    for family, engines in family_data.items():
        for engine, models in engines.items():
            ...
            if not has_any:
                continue
            ax.set_title(f"{family} - {engine} - Energy per Query CDF", weight="bold")
            ax.set_xscale('log')
            ax.legend(loc='lower right')
            family_engine_dir = os.path.join(base_plots_dir, family.lower(), engine)
            save(fig, family_engine_dir, f"energy_cdf_{engine}.png")
    ...
    for family, engines in family_data.items():
        for engine, models in engines.items():
            ...
            for model_name, dfs in models.items():
                ...
                ax.fill_between(iters, mean_e - std_e, mean_e + std_e, alpha=0.2, color=color)
            ax.set_ylabel("Energy (Joules)")
            ax.set_title(f"{family} - {engine} - Energy per Prompt (Mean ± Std)", weight="bold")
            ax.legend(loc='upper left', bbox_to_anchor=(1, 1), frameon=False)
            family_engine_dir = os.path.join(base_plots_dir, family.lower(), engine)
            save(fig, family_engine_dir, f"energy_per_prompt_{engine}.png")
...
def main():
    ...
    # Aggregated Logic
    if args.aggregate:
        aggregated_dir = os.path.join(base_plots, "aggregated")
        if args.accuracy or args.all: plot_accuracy_aggregated(cfg, base_results, aggregated_dir)
        if args.accuracy_by_type or args.all: plot_accuracy_by_type_aggregated(cfg, base_results, aggregated_dir, cmap)
        if args.latency or args.all: plot_latency_aggregated(cfg, base_results, aggregated_dir, cmap)
        if args.power or args.all: plot_power_aggregated(cfg, base_results, aggregated_dir, cmap)
        if args.temp or args.all: plot_temperature_aggregated(cfg, base_results, aggregated_dir, cmap)
    ...
    if args.family:
        ...
        if args.accuracy or args.all: plot_accuracy_family(family_data, family_root, cmap)
